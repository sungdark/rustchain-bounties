...
async function request(path, method = "GET", data) {
    for (const node of NODES) {
        try {
            ...
            console.warn(`Node ${node} failed: ${e.message}, trying next...`);
        }
    }
    ...
    try {
        switch (req.params.name) {
            case "rustchain_balance": {
                const { miner_id } = req.params.arguments;
            ...
            case "rustchain_health": {
                ...
                for (const node of NODES) {
                    try {
                        ...
                        healthStatus.push({ node, status: "unhealthy", error: e.message });
                    }
                }
                return {
            ...
            case "rustchain_transfer": {
                const { from_wallet, to_wallet, amount, private_key } = req.params.arguments;
            ...
            case "rustchain_ledger": {
                const { wallet_id } = req.params.arguments;
